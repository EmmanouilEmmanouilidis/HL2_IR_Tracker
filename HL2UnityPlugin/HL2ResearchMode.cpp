...
namespace winrt::HL2UnityPlugin::implementation
{
    ...
    float HL2ResearchMode::GetDepthOffset() 
    {
        return this->m_depthOffset;
    }

    
    com_array<float> HL2ResearchMode::GetUVPlane()
    {
        //std::lock_guard<std::mutex> l(mu);
        try
        {
            ...
            for (UINT i = 0; i < resolution.Height; i++)
            {
                for (UINT j = 0; j < resolution.Width; j++)
                {
                    if (i < resolution.Height && j < resolution.Width)
                    {
                        float xy[2] = {0, 0};
                        float uv[2] = {j, i};
                        this->m_pDepthCameraSensor->MapImagePointToCameraUnitPlane(uv, xy);
                        uvMap.push_back(xy[0]);
                        uvMap.push_back(xy[1]);
                    }
                }
            }
            //com_array<float> tempBuffer = com_array<FLOAT>(std::move_iterator(uvMap), std::move_iterator(uvMap));
            com_array<float> uvArray = com_array<float>(uvMap);
    ...
    com_array<float> HL2ResearchMode::GetWorldPosition(int64_t x, int64_t y)
    {
        std::lock_guard<std::mutex> l(mu);
        ...
        try {
            ...
            const UINT16* pDepth = nullptr;
            pDepthFrame->GetBuffer(&pDepth, &outBufferCount);
            this->m_depthBufferSize = outBufferCount;
            size_t outAbBufferCount = 0;
            const UINT16* pAbImage = nullptr;
            pDepthFrame->GetAbDepthBuffer(&pAbImage, &outAbBufferCount);
            ...
            auto ts = PerceptionTimestampHelper::FromSystemRelativeTargetTime(HundredsOfNanoseconds(checkAndConvertUnsigned(timestamp.HostTicks)));
            transToWorld = m_locator.TryLocateAtTimestamp(ts, m_refFrame);

            auto idx = resolution.Width * x + y;
            UINT16 depth = pDepth[idx];
            depth = (depth > 4090) ? 0 : depth - m_depthOffset;

            XMMATRIX depthToWorld = XMMatrixIdentity();
            
            depthToWorld = m_depthCameraPoseInvMatrix * SpatialLocationToDxMatrix(transToWorld);
            ...
            std::vector<float> tmp_pnt;
            tmp_pnt.push_back(XMVectorGetX(pointInWorld));
            tmp_pnt.push_back(XMVectorGetY(pointInWorld));
            tmp_pnt.push_back(-XMVectorGetZ(pointInWorld));
            com_array<float> position = com_array<float>( tmp_pnt);

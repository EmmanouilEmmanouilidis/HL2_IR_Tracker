...
def tcp_server():
    ...
    serverPort = 9090
    save_folder = 'C:/Users/alr_admin/HoloLens/HoloLens2-ResearchMode-Unity-master/python/HL2/'
    ...
    while True:
        ...
        try:
            data = conn.recv(2*512*512*4+100)
            if len(data)==0:
                continue
            ...
            if header == "m":
                ...
                #d_np = data[5:5+N]
                #print(d_np)
                map_img_np = np.frombuffer(data[5:5+N*4], dtype=np.float32) #.reshape((512, 512, 2))
                print(map_img_np.shape)
                timestamp = str(int(time.time()))
                #print(timestamp)
                map_file = open(save_folder + timestamp + '_uvMap.txt', "w")
                #for row in map_img_np:
                np.savetxt(map_file, map_img_np)
                map_file.close()
        except:
            break
